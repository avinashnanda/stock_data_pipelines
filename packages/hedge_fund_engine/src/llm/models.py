...
def get_model(model_name: str, model_provider: ModelProvider, api_keys: dict = None) -> ChatOpenAI | ChatGroq | ChatOllama | GigaChat | None:
    ...
    elif model_provider in ("LMStudio", "LMSTUDIO"):
        base_url = (api_keys or {}).get("LMSTUDIO_BASE_URL") or os.getenv("LMSTUDIO_BASE_URL", "http://localhost:1234/v1")
        return ChatOpenAI(model=model_name, api_key="lm-studio", base_url=base_url)
    elif model_provider in ("Custom", "CUSTOM"):
        base_url = (api_keys or {}).get(f"CUSTOM_BASE_URL_{model_name}") or os.getenv("CUSTOM_BASE_URL", "http://localhost:8000/v1")
        api_key = (api_keys or {}).get(f"CUSTOM_API_KEY_{model_name}") or os.getenv("CUSTOM_API_KEY", "no-key")
        return ChatOpenAI(model=model_name, api_key=api_key, base_url=base_url)
    else:
        raise ValueError(
            f"Unsupported model provider: {model_provider}. "
            f"Supported providers: {', '.join(p.value for p in ModelProvider)}, LMStudio, Custom"
        )
